 ...
 * 幾乎都只講 `assertThat(actual, matcher)`，但 `assertThat(reason, actual, matcher)` 則很少被提到?? => https://code.google.com/archive/p/hamcrest/wikis/Tutorial.wiki 有說一個 test 有多個 assert 時可以用?
